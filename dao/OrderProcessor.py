...
class OrderProcessor(OrderProcessorRepository):
    ...
    def createProduct(self, product):
        try:
            ...
            cursor.execute("INSERT INTO products (product_id,name, description, price, stock_quantity) VALUES (?,?, ?, ?, ?)",
                           (product.get_product_id(),product.get_name(), product.get_description(), product.get_price(), product.get_stock_quantity()))
            self.conn.commit()
    ...
    def placeOrder(self, order_id, customer, products_quantity_map, shippingAddress):
        try:
            ...
            cursor.execute(
                "INSERT INTO orders (order_id, customer_id, order_date, total_price, shipping_address) VALUES (?, ?, GETDATE(), ?, ?)",
                (order_id, customer.get_customer_id(), total_price, shippingAddress)
            )
